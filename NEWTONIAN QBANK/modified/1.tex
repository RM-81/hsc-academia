...
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $450\text{ kg}$ 
    \item[\textbf{B)}]  $350\text{ kg}$ 
    \item[\textbf{C)}]  $500\text{ kg}$ 
    \item[\textbf{D)}]  $400\text{ kg}$ 
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $450\text{ kg}$ 
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $a_{\text{cm}} = 3.27\text{ m/s}^2$
    \item[\textbf{B)}]  $a_{\text{cm}} = 4.90\text{ m/s}^2$
    \item[\textbf{C)}]  $a_{\text{cm}} = 2.45\text{ m/s}^2$
    \item[\textbf{D)}]  $a_{\text{cm}} = 1.96\text{ m/s}^2$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $a_{\text{cm}} = 3.27\text{ m/s}^2$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $v = \sqrt{\frac{g}{L} (L^2 - y_0^2)}$
    \item[\textbf{B)}] $v = \sqrt{g (L - y_0)}$
    \item[\textbf{C)}]  $v = \sqrt{\frac{2g}{L} (L^2 - y_0^2)}$
    \item[\textbf{D)}]  $v = \sqrt{\frac{g}{2L} (L^2 - y_0^2)}$
\end{itemize}
...
\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $v = \sqrt{\frac{g}{L} (L^2 - y_0^2)}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $552.92\text{ N}$
    \item[\textbf{B)}]  $376.99\text{ N}$
    \item[\textbf{C)}]  $502.65\text{ N}$
    \item[\textbf{D)}]  $425.40\text{ N}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $552.92\text{ N}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $80\text{ W}$
    \item[\textbf{B)}] $40\text{ W}$
    \item[\textbf{C)}]  $160\text{ W}$
    \item[\textbf{D)}]  $20\text{ W}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $80\text{ W}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $423.0\text{ MW}$
    \item[\textbf{B)}]  $623.8\text{ MW}$
    \item[\textbf{C)}] $300.0\text{ MW}$
    \item[\textbf{D)}]  $207.9\text{ MW}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $423.0\text{ MW}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}]  $3.68\text{ ms}^{-1}$
\item[\textbf{B)}]  $6.06\text{ ms}^{-1}$
\item[\textbf{C)}] $5.00\text{ ms}^{-1}$
\item[\textbf{D)}]  $7.36\text{ ms}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A)  $3.68\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
\noindent
ত্রিমাত্রিক স্থানাঙ্ক ব্যবস্থায় একটি পরিবর্তনশীল ভরের বস্তুর ভর এবং অবস্থান ভেক্টর সময়ের অপেক্ষক হিসেবে যথাক্রমে $m(t) = 10 - 0.5t^2\text{ kg}$ এবং $\vec{r}(t) = (2t^3)\hat{i} + (3t^2 - 4t)\hat{j} + (5\cos(\pi t))\hat{k}\text{ m}$ ধরে পরিবর্তিত হচ্ছে। $t = 2\text{ s}$ সময়ে বস্তুটির ওপর প্রযুক্ত লব্ধি বলের মান কত? ($\pi^2 \approx 9.87$ ধরুন)
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
In a three-dimensional coordinate system, the mass and position vector of a variable-mass object change with respect to time as $m(t) = 10 - 0.5t^2\text{ kg}$ and $\vec{r}(t) = (2t^3)\hat{i} + (3t^2 - 4t)\hat{j} + (5\cos(\pi t))\hat{k}\text{ m}$. At time $t = 2\text{ s}$, calculate the magnitude of the net force acting on the object. (Take $\pi^2 \approx 9.87$)
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] 421.46 N
\item[\textbf{B)}] 385.20 N
\item[\textbf{C)}] 450.12 N
\item[\textbf{D)}] 312.80 N
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A) 421.46 N
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: বেগ ও ত্বরণ ভেক্টর নির্ণয়} \\
অবস্থান ভেক্টরকে ব্যবকলন করে বেগ ভেক্টর:
\[\vec{v}(t) = \frac{d\vec{r}}{dt} = (6t^2)\hat{i} + (6t - 4)\hat{j} - (5\pi\sin(\pi t))\hat{k}\]

বেগ ভেক্টরকে ব্যবকলন করে ত্বরণ ভেক্টর:
\[\vec{a}(t) = \frac{d\vec{v}}{dt} = (12t)\hat{i} + 6\hat{j} - (5\pi^2\cos(\pi t))\hat{k}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: $t = 2\text{ s}$ সময়ে প্রয়োজনীয় রাশিমালার মান নির্ণয়} \\
\begin{align*}
    m(2) &= 10 - 0.5(2)^2 = 8\text{ kg} \\
    \frac{dm}{dt} &= -t \implies \left.\frac{dm}{dt}\right|_{t=2} = -2\text{ kg/s} \\
    \vec{v}(2) &= 6(2)^2\hat{i} + (6(2) - 4)\hat{j} - 5\pi\sin(2\pi)\hat{k} = 24\hat{i} + 8\hat{j}\text{ ms}^{-1} \\
    \vec{a}(2) &= 12(2)\hat{i} + 6\hat{j} - 5\pi^2\cos(2\pi)\hat{k} \approx 24\hat{i} + 6\hat{j} - 5(9.87)(1)\hat{k} = 24\hat{i} + 6\hat{j} - 49.35\hat{k}\text{ ms}^{-2}
\end{align*}

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: লব্ধি বলের মান নির্ণয়} \\
পরিবর্তনশীল ভরের ক্ষেত্রে নিউটনের দ্বিতীয় সূত্রানুসারে:
\[\vec{F} = m\vec{a} + \vec{v}\frac{dm}{dt}\]

মান বসিয়ে পাই:
\begin{align*}
    \vec{F} &= 8(24\hat{i} + 6\hat{j} - 49.35\hat{k}) + (-2)(24\hat{i} + 8\hat{j}) \\
    \vec{F} &= (192 - 48)\hat{i} + (48 - 16)\hat{j} - 394.8\hat{k} \\
    \vec{F} &= 144\hat{i} + 32\hat{j} - 394.8\hat{k}\text{ N}
\end{align*}

অতএব, বলের মান:
\[|\vec{F}| = \sqrt{144^2 + 32^2 + (-394.8)^2} = \sqrt{20736 + 1024 + 155867.04} \approx \mathbf{421.46\text{ N}}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 9
% ------------------------------------------
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
    \noindent
    \textbf{ধাপ ১: লিফটের ভেতর কার্যকর অভিকর্ষজ ত্বরণ নির্ণয়} \\
    যেহেতু লিফটটি $A_0 = 2.2\text{ ms}^{-2}$ ত্বরণে উপরের দিকে উঠছে, তাই লিফটের ভেতরের অজড়ীয় প্রসঙ্গ কাঠামোতে কার্যকর অভিকর্ষজ ত্বরণ হবে:
    \begin{align*}
        g_{\text{eff}} &= g + A_0 \\
        &= 9.8 + 2.2 \\
        &= 12.0\text{ ms}^{-2}
    ...
    \vspace{0.5em}
    \noindent\textbf{ধাপ ৩: সুতার টান নির্ণয়} \\
    সুতার টান হলো একটি অভ্যন্তরীণ বল যা বস্তুর ভরের উপর এবং তাদের আপেক্ষিক ত্বরণের উপর নির্ভর করে। এটি সকল রেফারেন্স ফ্রেমে (ভিতরের এবং বাইরের পর্যবেক্ষক উভয়ের কাছে) সমান থাকে। সুতার টানের সূত্র:
    \begin{align*}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $1.50\text{ ms}^{-1}$
    \item[\textbf{B)}] $2.10\text{ ms}^{-1}$
    \item[\textbf{C)}] $2.50\text{ ms}^{-1}$
    \item[\textbf{D)}]  $3.14\text{ ms}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $1.50\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
\noindent
$3\text{ kg}$ ভর এবং $1\text{ m}$ দৈর্ঘ্যের একটি সুষম রডের দুই প্রান্তে $2\text{ kg}$ ভর ও $0.2\text{ m}$ ব্যাসার্ধবিশিষ্ট দুটি অভিন্ন নিরেট গোলক দৃঢ়ভাবে যুক্ত করা হলো। সমগ্র সিস্টেমটিকে একটি গোলকের কেন্দ্র $O$-এর মধ্য দিয়ে রডের দৈর্ঘ্যের সাথে লম্বভাবে অতিক্রান্ত অক্ষের সাপেক্ষে পিভোট (pivot) করা হলে সমগ্র সিস্টেমটির জড়তার ভ্রামক ($I$) কত হবে?
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
A system consists of a uniform rod of mass $3\text{ kg}$ and length $1\text{ m}$ with two identical solid spheres of mass $2\text{ kg}$ and radius $0.2\text{ m}$ welded at its two ends. The system is pivoted at the center $O$ of one of the spheres about an axis perpendicular to the length of the rod. Determine the moment of inertia ($I$) of the system about the pivot axis.
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $2.85\text{ kg m}^2$
\item[\textbf{B)}] $3.12\text{ kg m}^2$
\item[\textbf{C)}] $4.00\text{ kg m}^2$
\item[\textbf{D)}] $3.32\text{ kg m}^2$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} D) $3.32\text{ kg m}^2$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: গোলক ১-এর জড়তার ভ্রামক নির্ণয়} \\
অক্ষটি গোলক ১-এর নিজ কেন্দ্রের মধ্য দিয়ে গমন করায়:
\[I_1 = \frac{2}{5}mR^2 = \frac{2}{5} \times 2 \times (0.2)^2 = 0.16\text{ kg m}^2\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: রডের জড়তার ভ্রামক নির্ণয়} \\
রডের এক প্রান্ত অক্ষের সাথে যুক্ত থাকায় (ভরকেন্দ্র $O$ থেকে $\frac{L}{2} = 0.5\text{ m}$ দূরে) সমান্তরাল অক্ষ উপপাদ্য প্রয়োগ করে:
\[I_{\text{rod}} = \frac{1}{12}ML^2 + M\left(\frac{L}{2}\right)^2 = \frac{1}{3}ML^2 = \frac{1}{3} \times 3 \times (1)^2 = 1.0\text{ kg m}^2\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: গোলক ২-এর জড়তার ভ্রামক নির্ণয়} \\
গোলক ২-এর কেন্দ্র পিভোট অক্ষ $O$ থেকে $d = L = 1.0\text{ m}$ দূরে অবস্থিত হওয়ায় সমান্তরাল অক্ষ উপপাদ্যানুসারে:
\[I_2 = \frac{2}{5}mR^2 + mL^2 = 0.16 + 2 \times (1)^2 = 2.16\text{ kg m}^2\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৪: সমগ্র ব্যবস্থার মোট জড়তার ভ্রামক নির্ণয়} \\
সমগ্র ব্যবস্থার মোট জড়তার ভ্রামক:
\[I = I_1 + I_{\text{rod}} + I_2 = 0.16 + 1.0 + 2.16 = \mathbf{3.32\text{ kg m}^2}\]
\end{tcolorbox}
% ------------------------------------------
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $a = 3.27\text{ ms}^{-2}$
    \item[\textbf{B)}]  $a = 1.96\text{ ms}^{-2}$
    \item[\textbf{C)}]  $a = 2.45\text{ ms}^{-2}$
    \item[\textbf{D)}]  $a = 4.90\text{ ms}^{-2}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $a = 3.27\text{ ms}^{-2}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $\theta = \arccos\left(\frac{1}{2}\right) = 60^\circ$
    \item[\textbf{B)}]$\theta = \arccos\left(\frac{3}{4}\right) \approx 41.4^\circ$
    \item[\textbf{C)}]  $\theta = \arccos\left(\frac{5}{6}\right) \approx 33.6^\circ$
    \item[\textbf{D)}]  $\theta = \arccos\left(\frac{2}{3}\right) \approx 48.2^\circ$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} C)  $\theta = \arccos\left(\frac{5}{6}\right) \approx 33.6^\circ$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $\frac{F}{3}$ (বল $F$-এর দিকে অর্থাৎ সম্মুখমুখী)
    \item[\textbf{B)}]  $\frac{F}{3}$ (বল $F$-এর বিপরীত দিকে অর্থাৎ পশ্চাৎমুখী)
    \item[\textbf{C)}]  শূন্য
    \item[\textbf{D)}]  $\frac{F}{2}$ (সম্মুখমুখী)
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $\frac{F}{3}$ (বল $F$-এর দিকে অর্থাৎ সম্মুখমুখী)
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $3.27\text{ kg}$
    \item[\textbf{B)}] $2.00\text{ kg}$
    \item[\textbf{C)}]  $4.80\text{ kg}$
    \item[\textbf{D)}]  $1.63\text{ kg}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $3.27\text{ kg}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $14.0\text{ ms}^{-1}$
    \item[\textbf{B)}]  $9.8\text{ ms}^{-1}$
    \item[\textbf{C)}]  $19.6\text{ ms}^{-1}$
    \item[\textbf{D)}]  $22.4\text{ ms}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $14.0\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $128\text{ J}$
    \item[\textbf{B)}]  $64\text{ J}$
    \item[\textbf{C)}]  $96\text{ J}$
    \item[\textbf{D)}]  $160\text{ J}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $128\text{ J}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $4.9\text{ ms}^{-2}$
    \item[\textbf{B)}]  $9.8\text{ ms}^{-2}$
    \item[\textbf{C)}]  $0\text{ ms}^{-2}$
    \item[\textbf{D)}]  $2.45\text{ ms}^{-2}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $4.9\text{ ms}^{-2}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $13.29\text{ ms}^{-2}$
    \item[\textbf{B)}] $10.20\text{ ms}^{-2}$
    \item[\textbf{C)}]  $15.45\text{ ms}^{-2}$
    \item[\textbf{D)}]  $8.50\text{ ms}^{-2}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $13.29\text{ ms}^{-2}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $2.0\text{ ms}^{-1}$
    \item[\textbf{B)}]  $1.5\text{ ms}^{-1}$
    \item[\textbf{C)}] $2.5\text{ ms}^{-1}$
    \item[\textbf{D)}] $0\text{ ms}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $2.0\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]$\frac{7}{18} M L^2$
    \item[\textbf{B)}] $\frac{5}{12} M L^2$
    \item[\textbf{C)}]  $\frac{2}{5} M L^2$
    \item[\textbf{D)}]  $\frac{3}{7} M L^2$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $\frac{7}{18} M L^2$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $\frac{181}{2} M R^2$
    \item[\textbf{B)}]  $\frac{55}{2} M R^2$
    \item[\textbf{C)}]  $\frac{73}{2} M R^2$
    \item[\textbf{D)}]  $90 M R^2$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $\frac{181}{2} M R^2$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $3 M R^2$
    \item[\textbf{B)}]  $\frac{5}{2} M R^2$
    \item[\textbf{C)}]  $2 M R^2$
    \item[\textbf{D)}]$\frac{7}{2} M R^2$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $3 M R^2$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $12.87\text{ ms}^{-1}$
    \item[\textbf{B)}] $16.82\text{ ms}^{-1}$
    \item[\textbf{C)}] $9.50\text{ ms}^{-1}$
    \item[\textbf{D)}] $14.25\text{ ms}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $12.87\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $\sqrt{2} : \sqrt{3}$
    \item[\textbf{B)}] $\sqrt{3} : \sqrt{2}$
    \item[\textbf{C)}]  $2 : 3$
    \item[\textbf{D)}]  $1 : 1$
\end{itemize}
...
\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $\sqrt{2} : \sqrt{3}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $F = (M + m)g \tan\theta$
    \item[\textbf{B)}]  $F = (M + m)g \cot\theta$
    \item[\textbf{C)}]  $F = mg \sin\theta$
    \item[\textbf{D)}]  $F = M g \tan\theta$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $F = (M + m)g \tan\theta$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $36\sqrt{409}\text{ Nm}$
    \item[\textbf{B)}] $540\text{ Nm}$
    \item[\textbf{C)}] $18\sqrt{305}\text{ Nm}$
    \item[\textbf{D)}] $720\text{ Nm}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $36\sqrt{409}\text{ Nm}$
\end{tcolorbox}
\vspace{1em}
...
\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $66.67\%$
\end{tcolorbox}
\vspace{1em}
...
\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $2mg$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $0\text{ N}$
    \item[\textbf{B)}] $588\text{ N}$
    \item[\textbf{C)}]  $9800\text{ N}$
    \item[\textbf{D)}] $1176\text{ N}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $0\text{ N}$
\end{tcolorbox}
\vspace{1em}
...
\noindent
একটি তারকে $y = k x^2$ পরাবৃত্তের আকৃতিতে বাঁকানো হলো এবং এর ওপর একটি $m$ ভরের মার্বেল রাখা আছে। তারটিকে X-অক্ষের সমান্তরালে $a$ ত্বরণে গতিশীল করা হলে, মূলবিন্দু থেকে কত অনুভূমিক দূরত্বে মার্বেলটি সাম্যাবস্থায় স্থির থাকবে?
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
A wire is bent in the shape of a parabola $y = k x^2$. A marble of mass $m$ is placed on it. If the wire is accelerated horizontally with an acceleration $a$, at what horizontal distance $x$ from the origin will the marble remain at relative rest?
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $x = \frac{a}{gk}$
\item[\textbf{B)}] $x = \frac{a}{2gk}$
\item[\textbf{C)}] $x = \frac{2a}{gk}$
\item[\textbf{D)}] $x = \frac{a}{4gk}$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} B) $x = \frac{a}{2gk}$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: স্পর্শকের ঢাল নির্ণয়} \\
পরাবৃত্তাকার তারের যেকোনো বিন্দুতে স্পর্শকের ঢাল:
\[\tan\theta = \frac{dy}{dx} = \frac{d}{dx}(kx^2) = 2kx\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: সাম্যাবস্থার শর্ত প্রয়োগ} \\
ত্বরণশীল (অ-জড়তা) ফ্রেমে মার্বেলের ওপর কাজ করা বলগুলোর সমীকরণ:
\begin{align*}
    R \cos\theta &= mg \\
    R \sin\theta &= ma
\end{align*}
...
\vspace{0.5em}
\noindent\textbf{ধাপ ৩: মূলবিন্দু থেকে অনুভূমিক দূরত্ব $x$ নির্ণয়} \\
দ্বিতীয় সমীকরণকে প্রথম সমীকরণ দ্বারা ভাগ করে পাই:
\[\tan\theta = \frac{ma}{mg} = \frac{a}{g}\]

ঢালের মান বসিয়ে:
\[2kx = \frac{a}{g} \implies \mathbf{x = \frac{a}{2gk}}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 33
% ------------------------------------------
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $8.0\text{ N}$
    \item[\textbf{B)}]  $480\text{ N}$
    \item[\textbf{C)}]  $80.0\text{ N}$
    \item[\textbf{D)}]  $4.0\text{ N}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $8.0\text{ N}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $\frac{m^2 u^2}{2\mu g (M + m)^2}$
    \item[\textbf{B)}]  $\frac{m u^2}{2\mu g (M + m)}$
    \item[\textbf{C)}]  $\frac{m^2 u^2}{2\mu g M^2}$
    \item[\textbf{D)}]  $\frac{(M + m) u^2}{2\mu g m}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $\frac{m^2 u^2}{2\mu g (M + m)^2}$
\end{tcolorbox}
\vspace{1em}
...
\noindent
প্রত্যেকটি $M = 1.5\text{ kg}$ ভর ও $L = 0.8\text{ m}$ দৈর্ঘ্যের চারটি অভিন্ন সরু সুষম রড দিয়ে একটি বর্গাকার ফ্রেম তৈরি করা হলো। ফ্রেমটির চার কোণায় $m_0 = 0.5\text{ kg}$ ভরের চারটি ছোট গোলক যুক্ত আছে। ফ্রেমটির যেকোনো একটি কোণাগামী এবং তলের সাথে লম্ব অক্ষের সাপেক্ষে মোট জড়তার ভ্রামক কত?
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
A square frame is made of four identical thin uniform rods of mass $M = 1.5\text{ kg}$ and length $L = 0.8\text{ m}$. Four point masses $m_0 = 0.5\text{ kg}$ are attached to the corners. Calculate the total moment of inertia about an axis passing through one corner perpendicular to the frame.
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $4.48\text{ kg m}^2$
\item[\textbf{B)}] $3.84\text{ kg m}^2$
\item[\textbf{C)}] $5.12\text{ kg m}^2$
\item[\textbf{D)}] $2.96\text{ kg m}^2$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A) $4.48\text{ kg m}^2$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: সংলগ্ন দুটি রডের জড়তার ভ্রামক নির্ণয়} \\
অক্ষগামী দুটি সংলগ্ন রডের প্রতিটি এক প্রান্তের সাপেক্ষে ঘোরে:
\[I_1 = 2 \times \left(\frac{1}{3} M L^2\right) = \frac{2}{3} (1.5) (0.8)^2 = 0.64\text{ kg m}^2\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: বিপরীত দুটি রডের জড়তার ভ্রামক নির্ণয়} \\
অপর দুটি বিপরীত রডের ক্ষেত্রে সমান্তরাল অক্ষ উপপাদ্য প্রয়োগ করে:
\[I_2 = 2 \times \left(\frac{1}{3} M L^2 + M L^2\right) = \frac{8}{3} M L^2 = \frac{8}{3} (1.5) (0.8)^2 = 2.56\text{ kg m}^2\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: চার কোণার ভরগুলোর জড়তার ভ্রামক নির্ণয়} \\
অক্ষে অবস্থিত ভর, সংলগ্ন দুটি ভর এবং বিপরীত কোণার ভরের জড়তার ভ্রামক:
\begin{align*}
    I_{\text{spheres}} &= m_0(0)^2 + 2 \times m_0 L^2 + m_0 (\sqrt{2}L)^2 \\
    &= 4 m_0 L^2 = 4(0.5)(0.8)^2 = 1.28\text{ kg m}^2
\end{align*}

\vspace{0.5em}
\noindent\textbf{ধাপ ৪: মোট জড়তার ভ্রামক নির্ণয়} \\
সমগ্র ব্যবস্থার মোট জড়তার ভ্রামক:
\[I = I_1 + I_2 + I_{\text{spheres}} = 0.64 + 2.56 + 1.28 = \mathbf{4.48\text{ kg m}^2}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 36
% ------------------------------------------
...
\noindent
একটি বস্তুকণা $y = \frac{x^3}{6}$ সমীকরণ মেনে চলা একটি উল্লম্ব তলের ওপর অবস্থান করছে। বস্তু ও তলের মধ্যকার স্থিতি ঘর্ষণ গুণাঙ্ক $\mu_s = 0.5$ হলে, ভূমি থেকে সর্বোচ্চ কত উচ্চতায় বস্তুটি স্লিপ না করে স্থির থাকতে পারবে?
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
A particle rests on a vertical curved surface given by $y = \frac{x^3}{6}$. If the coefficient of static friction is $\mu_s = 0.5$, what is the maximum height $h$ above the ground at which the particle can remain stationary without slipping?
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $\frac{1}{3}\text{ m}$
\item[\textbf{B)}] $\frac{1}{2}\text{ m}$
\item[\textbf{C)}] $\frac{1}{4}\text{ m}$
\item[\textbf{D)}] $\frac{1}{6}\text{ m}$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} D) $\frac{1}{6}\text{ m}$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: আনতি কোণের ঢাল নির্ণয়} \\
বক্ররেখার যেকোনো বিন্দুতে আনতি কোণের ঢাল:
\[\tan\theta = \frac{dy}{dx} = \frac{d}{dx}\left(\frac{x^3}{6}\right) = \frac{3x^2}{6} = \frac{x^2}{2}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: স্লিপ না করার সীমাস্থ অবস্থা থেকে $x$ নির্ণয়} \\
স্লিপ না করে স্থির থাকার সীমাস্থ অবস্থায়:
\[\tan\theta = \mu_s \implies \frac{x^2}{2} = 0.5 \implies x^2 = 1 \implies x = 1\text{ m}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: সর্বোচ্চ উচ্চতা $h$ নির্ণয়} \\
$x = 1\text{ m}$ মান বক্ররেখার সমীকরণে বসিয়ে পাই:
\[h = y = \frac{x^3}{6} = \frac{1^3}{6} = \mathbf{\frac{1}{6}\text{ m}}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 37
% ------------------------------------------
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] মোট ঘাত $16\text{ N s}$ এবং সর্বোচ্চ বেগ $8\text{ ms}^{-1}$
\item[\textbf{B)}] মোট ঘাত $24\text{ N s}$ এবং সর্বোচ্চ বেগ $12\text{ ms}^{-1}$
\item[\textbf{C)}] মোট ঘাত $16\text{ N s}$ এবং সর্বোচ্চ বেগ $4\text{ ms}^{-1}$
\item[\textbf{D)}] মোট ঘাত $8\text{ N s}$ এবং সর্বোচ্চ বেগ $8\text{ ms}^{-1}$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A) মোট ঘাত $16\text{ N s}$ এবং সর্বোচ্চ বেগ $8\text{ ms}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
বল শূন্য হওয়ার মুহূর্ত:
\[F(t) = 0 \implies 12 - 3t^2 = 0 \implies 3t^2 = 12 \implies t = 2\text{ s}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: মোট ঘাত (Impulse) নির্ণয়} \\
$t = 0$ থেকে $t = 2\text{ s}$ পর্যন্ত বলের মোট ঘাত $J$:
\[J = \int_0^2 F(t) \, dt = \int_0^2 (12 - 3t^2) \, dt = \left[12t - t^3\right]_0^2 = (24 - 8) - 0 = 16\text{ N s}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: বস্তুর অর্জিত সর্বোচ্চ বেগ নির্ণয়} \\
ঘাত-ভরবেগ উপপাদ্য অনুসারে ($v_{\text{initial}} = 0$):
\[J = \Delta p = m v_{\text{max}} \implies v_{\text{max}} = \frac{J}{m} = \frac{16}{2} = \mathbf{8\text{ ms}^{-1}}\]
\end{tcolorbox}
% ------------------------------------------
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $24\text{ J}$
    \item[\textbf{B)}]  $16\text{ J}$
    \item[\textbf{C)}]  $32\text{ J}$
    \item[\textbf{D)}]  $48\text{ J}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $24\text{ J}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $540.00\text{ Nm}$
\item[\textbf{B)}] $36\sqrt{409}\text{ Nm}$
\item[\textbf{C)}] $720.00\text{ Nm}$
\item[\textbf{D)}] $18\sqrt{305}\text{ Nm}$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} B) $36\sqrt{409}\text{ Nm}$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: বেগ, ত্বরণ ও বল ভেক্টর নির্ণয়} \\
সময়ের সাপেক্ষে কণার অবস্থান ভেক্টর:
\[\vec{r}(t) = (2t^2)\hat{i} + (3t)\hat{j} - (5t^3)\hat{k}\]
...
$m = 3\text{ kg}$ ভরের ওপর প্রযুক্ত বল:
\[\vec{F}(t) = m\vec{a}(t) = 3[4\hat{i} - (30t)\hat{k}] = 12\hat{i} - (90t)\hat{k}\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: $t = 2\text{ s}$ সময়ে অবস্থান ও বল ভেক্টর নির্ণয়} \\
\begin{align*}
    \vec{r}(2) &= 2(2)^2\hat{i} + 3(2)\hat{j} - 5(2)^3\hat{k} = 8\hat{i} + 6\hat{j} - 40\hat{k}\text{ m} \\
    \vec{F}(2) &= 12\hat{i} + 0\hat{j} - 90(2)\hat{k} = 12\hat{i} + 0\hat{j} - 180\hat{k}\text{ N}
\end{align*}

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: মূলবিন্দুর সাপেক্ষে টর্ক ও তার মান নির্ণয়} \\
টর্ক $\vec{\tau} = \vec{r} \times \vec{F}$:
\[\vec{\tau} = \begin{vmatrix} \hat{i} & \hat{j} & \hat{k} \\ 8 & 6 & -40 \\ 12 & 0 & -180 \end{vmatrix}\]

নির্ণায়ক প্রসারিত করে:
\begin{align*}
    \vec{\tau} &= \hat{i}[6(-180) - 0] - \hat{j}[8(-180) - (-40)(12)] + \hat{k}[8(0) - 6(12)] \\
    &= -1080\hat{i} + 960\hat{j} - 72\hat{k}\text{ Nm}
\end{align*}

টর্কের মান:
\[|\vec{\tau}| = \sqrt{(-1080)^2 + (960)^2 + (-72)^2} = \sqrt{2093184} = \mathbf{36\sqrt{409}\text{ Nm}}\]
\end{tcolorbox}
% ------------------------------------------
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $15.81\text{ km}$
    \item[\textbf{B)}]  $12.45\text{ km}$
    \item[\textbf{C)}] $18.50\text{ km}$
    \item[\textbf{D)}]  $10.20\text{ km}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $15.81\text{ km}$
\end{tcolorbox}
\vspace{1em}
...
\begin{english}
\noindent\textit{\color{darkgray}
In a gravity-free deep space environment, a rocket has an initial total mass of $M_0 = 8000\text{ kg}$. The exhaust gas relative speed is $v_r = 2500\text{ ms}^{-1}$. To accelerate the rocket from rest to $5.0\text{ kms}^{-1}$, what percentage of its initial mass must be burned as fuel?
}
\end{english}
\end{tcolorbox}
...
\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} B) $86.47\%$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: ৎসিলকোভস্কি রকেট সমীকরণ প্রয়োগ} \\
রকেটের বেগ বৃদ্ধির সমীকরণ:
\[\Delta v = v_r \ln\left(\frac{M_0}{M}\right)\]

এখানে, $\Delta v = 5.0\text{ kms}^{-1} = 5000\text{ ms}^{-1}$ এবং $v_r = 2500\text{ ms}^{-1}$। মান বসিয়ে পাই:
\[5000 = 2500 \ln\left(\frac{M_0}{M}\right) \implies \ln\left(\frac{M_0}{M}\right) = 2 \implies \frac{M_0}{M} = e^2 \approx 7.389\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: অবশিষ্ট ভর ও ব্যবহৃত জ্বালানির ভর নির্ণয়} \\
অবশিষ্ট ভর:
\[M = \frac{M_0}{e^2} = M_0 e^{-2} \approx 0.13533 M_0\]

ব্যবহৃত জ্বালানির ভর $\Delta m$:
\[\Delta m = M_0 - M = M_0(1 - e^{-2}) = M_0(1 - 0.13533) = 0.86467 M_0\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: জ্বালানির শতকরা পরিমাণ নির্ণয়} \\
আদি ভরের শতকরা কত অংশ জ্বালানি হিসেবে পুড়ল:
\[\text{শতকরা হার} = \frac{\Delta m}{M_0} \times 100\% = 0.86467 \times 100\% \approx \mathbf{86.47\%}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 42
% ------------------------------------------
...
\noindent
$M = 2.0\text{ kg}$ ভর এবং $L = 1.5\text{ m}$ দৈর্ঘ্যের একটি পাতলা সুষম দণ্ড মসৃণ অনুভূমিক টেবিলে স্থির অবস্থায় আছে। $m = 1.0\text{ kg}$ ভরের একটি কণা $u = 10.0\text{ ms}^{-1}$ বেগে দণ্ডের লম্বভাবে কেন্দ্র হতে $d = 0.5\text{ m}$ দূরে স্থিতিস্থাপকভাবে ($e = 1.0$) ধাক্কা দিল। ধাক্কার পর দণ্ডের ভরকেন্দ্রের রৈখিক বেগ $v_c$ এবং দণ্ডের কৌণিক বেগ $\omega$ কত হবে?
\vspace{0.8em}
\begin{english}
\noindent\textit{\color{darkgray}
A uniform thin rod ($M = 2.0\text{ kg}, L = 1.5\text{ m}$) lies at rest on a smooth table. A particle ($m = 1.0\text{ kg}, u = 10.0\text{ ms}^{-1}$) strikes it elastically at $d = 0.5\text{ m}$ from its center. Calculate $v_c$ and $\omega$ of the rod after collision.
}
\end{english}
\end{tcolorbox}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $v_c = 3.00\text{ ms}^{-1}$ এবং $\omega = 8.00\text{ rad/s}$
\item[\textbf{B)}] $v_c = 4.62\text{ ms}^{-1}$ এবং $\omega = 12.31\text{ rad/s}$
\item[\textbf{C)}] $v_c = 5.00\text{ ms}^{-1}$ এবং $\omega = 10.00\text{ rad/s}$
\item[\textbf{D)}] $v_c = 2.50\text{ ms}^{-1}$ এবং $\omega = 6.15\text{ rad/s}$
\end{itemize}
\vspace{0.5em}

\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} B) $v_c = 4.62\text{ ms}^{-1}$ এবং $\omega = 12.31\text{ rad/s}$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
\noindent
\textbf{ধাপ ১: রৈখিক ভরবেগের সংরক্ষণশীলতা প্রয়োগ} \\
ধাক্কার পর কণার বেগ $v_p$ ধরে:
\[m u = m v_p + M v_c \implies 1(10) = 1(v_p) + 2 v_c \implies v_p = 10 - 2 v_c\]

\vspace{0.5em}
\noindent\textbf{ধাপ ২: কৌণিক ভরবেগের সংরক্ষণশীলতা প্রয়োগ} \\
দণ্ডের ভরকেন্দ্রের সাপেক্ষে জড়তার ভ্রামক:
\[I = \frac{1}{12} M L^2 = \frac{1}{12} \times 2.0 \times (1.5)^2 = 0.375\text{ kg m}^2\]

ভরকেন্দ্রের সাপেক্ষে কৌণিক ভরবেগ সংরক্ষণশীলতার নীতি বসিয়ে:
\[m u d = m v_p d + I \omega \implies 1 \times 10 \times 0.5 = 1 \times v_p \times 0.5 + 0.375 \omega\]
\[v_p + 0.75 \omega = 10 \implies (10 - 2 v_c) + 0.75 \omega = 10 \implies \omega = \frac{8}{3} v_c\]

\vspace{0.5em}
\noindent\textbf{ধাপ ৩: স্থিতিস্থাপক সংঘাতের সূত্র হতে $v_c$ ও $\omega$ নির্ণয়} \\
সংঘাতের বিন্দুতে আপেক্ষিক অপসরণ বেগ ও আপেক্ষিক আগমন বেগের অনুপাত ($e = 1.0$):
\[(v_c + \omega d) - v_p = u \implies v_c + 0.5\left(\frac{8}{3} v_c\right) - (10 - 2 v_c) = 10\]
\[\frac{13}{3} v_c = 20 \implies \mathbf{v_c = \frac{60}{13} \approx 4.62\text{ ms}^{-1}}\]

দণ্ডের কৌণিক বেগ:
\[\mathbf{\omega = \frac{8}{3} \left(\frac{60}{13}\right) = \frac{160}{13} \approx 12.31\text{ rad/s}}\]
\end{tcolorbox}
% ------------------------------------------
% QUESTION SECTION 43
% ------------------------------------------
...
\itemize
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $40.92\text{ N}$
    \item[\textbf{B)}] $36.00\text{ N}$
    \item[\textbf{C)}] $19.60\text{ N}$
    \item[\textbf{D)}] $52.14\text{ N}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $40.92\text{ N}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}]  $a = 2.00\text{ ms}^{-2}$
\item[\textbf{B)}]  $a = 3.00\text{ ms}^{-2}$
\item[\textbf{C)}]  $a = 1.50\text{ ms}^{-2}$
\item[\textbf{D)}]  $a = 4.50\text{ ms}^{-2}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A)  $a = 2.00\text{ ms}^{-2}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}] $a = 3.06\text{ m/s}^2$
    \item[\textbf{B)}] $a = 3.50\text{ m/s}^2$
    \item[\textbf{C)}] $a = 2.45\text{ m/s}^2$
    \item[\textbf{D)}] $a = 4.12\text{ m/s}^2$
\end{itemize}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
\item[\textbf{A)}] $\omega_2 = 9\text{ rpm}$
\item[\textbf{B)}] $\omega_2 = 7\text{ rpm}$
\item[\textbf{C)}] $\omega_2 = 12\text{ rpm}$
\item[\textbf{D)}] $\omega_2 = 14\text{ rpm}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
\textbf{\color{success} উত্তর:} A)  $\omega_2 = 9\text{ rpm}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $12.25\text{ N}$
    \item[\textbf{B)}]  $29.40\text{ N}$
    \item[\textbf{C)}]  $8.17\text{ N}$
    \item[\textbf{D)}]  $14.70\text{ N}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A)  $12.25\text{ N}$
\end{tcolorbox}
\vspace{1em}
...
% ------------------------------------------
\begin{itemize}[label={}, leftmargin=1cm, itemsep=0.5em]
    \item[\textbf{A)}]  $31.25\text{ rad s}^{-1}$
    \item[\textbf{B)}]  $25.00\text{ rad s}^{-1}$
    \item[\textbf{C)}]  $18.75\text{ rad s}^{-1}$
    \item[\textbf{D)}]  $37.50\text{ rad s}^{-1}$
\end{itemize}

\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $31.25\text{ rad s}^{-1}$
\end{tcolorbox}
\vspace{1em}
...
\vspace{0.5em}
\begin{tcolorbox}[answerbox]
    \textbf{\color{success} উত্তর:} A) $86.47\%$
\end{tcolorbox}
\vspace{1em}
...
\begin{tcolorbox}[solutionbox={সমাধান (Solution)}]
    \noindent
    \textbf{ধাপ ১: রকেট সমীকরণ প্রয়োগ} \\
    মহাশূন্যে রকেটের গতির জন্য সমীকরণটি হলো:
    \[ \Delta v = v_r \ln\left(\frac{M_0}{M}\right) \]
